...
from models.swin import BasicLayer2d
...
class MaskPredictor(nn.Module):
    """ Wraps Encoder, ProjectHeads and Classifier """
    def __init__(
            self,
            img_size: int = 256,                            # encoder
            dim: int = 64,                                  # encoder
            n_conv_stages: int = 0,                         # encoder
            dim_mults: List[int] = (1, 2, 4),               # encoder
            proj_dim: int = 128,                            # project heads & classifier
            fuse: bool = True,                              # project heads
            encoder_depths: List[int] = (6, 4, 2),          # encoder
            window_size: int = 8,                           # encoder
...
class InpaintNet(nn.Module):
    ...
    def __init__(
            ...
            window_size: int = 8,                           # encoder & decoder
            bottleneck_window_size: int = 16,               # bottleneck
            bottleneck_depth: int = 2,                      # bottleneck
